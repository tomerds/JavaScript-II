...
let fullName = [];
fullName = runners.map(e => e.first_name + ' ' + e.last_name);
...
let largeShirts = [];
largeShirts = runners.filter(function checkSize(e) {
...
ticketPriceTotal = runners.reduce((sum, e) => {
    return sum += e.donation;
}, 0)
...
function sizeCounter(size, array) {
    let sum = 0;
    for (let i = 0; i < array.length; i++) {
        if (size == array[i].shirt_size) {
            sum += 1;
        }
    }
    return sum;
...
function checkSizer(e, size) {
    return e.shirt_size === size;
}


sizes = runners.map(function (e) {

    return {
        L: e.filter(checkSizer(array, 'L')).length,
    }
